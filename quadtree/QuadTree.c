...
void ascent(quadtreeNode *node){
  printf("\n");
}

void descent(quadtreeNode *node){
  if(node->bounds != NULL)
...
int main() {
	...
	printf("tree->root->bounds->nw->x: %f \n", tree->root->bounds->nw->x);
	printf("tree->root->bounds->nw->y: %f \n", tree->root->bounds->nw->y);
	printf("tree->root->bounds->se->x: %f \n", tree->root->bounds->se->x);
	printf("tree->root->bounds->se->y: %f \n", tree->root->bounds->se->y);

	quadtreeInsert(tree, 0, 0, &key);
	quadtreeInsert(tree, 110.0, 110.0, &key);
	quadtreeInsert(tree, 8.0, 2.0, &key);
	printf("tree->length: %u\n", tree->length);
	printf("tree->root->point->x: %f\n", tree->root->point->x);
	printf("tree->root->point->y: %f\n", tree->root->point->y);

	quadtreeInsert(tree, 0.0, 1.0, &key); //Fail
	quadtreeInsert(tree, 2.0, 3.0, &key); //normal
	quadtreeInsert(tree, 2.0, 3.0, &key); //replacement
	printf("tree->length: %u\n", tree->length);
	quadtreeInsert(tree, 3.0, 1.1, &key);
	printf("tree->length: %u\n", tree->length);
	printf("quadtreeSearch: %f\n",quadtreeSearch(tree, 3.0, 1.1)->x);
	quadtreeWalk(tree->root, ascent, descent);

	//Test Points
	quadtreePoint *point = quadtreePointNew(5.0, 6.0);
	printf("point->x: %f\n", point->x);
	printf("point->y: %f\n", point->y);
...
static int splitNode(quadtree *tree, quadtreeNode *node) {
	...
	double hw = node->bounds->width / 2.0;
	double hh = node->bounds->height / 2.0;

	if(!(nw = quadtreeNodeWithBounds(x,    y-hh,   x+hw,   y))) return 0;
	if(!(ne = quadtreeNodeWithBounds(x+hw, y-hh,   x+hw*2.0, y))) return 0;
	if(!(sw = quadtreeNodeWithBounds(x,    y-hh*2.0, x+hw,   y-hh))) return 0;
	if(!(se = quadtreeNodeWithBounds(x+hw, y-hh*2.0, x+hw*2.0, y-hh))) return 0;
